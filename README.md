...
[![Version](https://img.shields.io/badge/Version-2.1.10-green.svg)](CHANGELOG.md)
...
| **State Management** | 123 Lib Modules (26.3k LOC) | PDCA state machine, workflow engine, automation control, audit, quality gates, intent detection, team coordination, Clean Architecture Domain/Infra/Application layers |
...
- **Integrated Enhancement — Clean Architecture + Defense-in-Depth + Invocation Contract (v2.1.10)** — Sprint 0~6 전량 구현. **Clean Architecture 4-Layer** (Domain ports 6 + guards 4 + rules + cc-regression 8 modules, Domain 의존성 0건 CI 강제, Application/Infrastructure 레이어 분리), **Port↔Adapter 매핑 6종 완결** (cc-payload→cc-bridge, state-store, regression-registry, audit-sink→telemetry, token-meter, docs-code-index), **Guard Registry** 21 guards + `lifecycle.reconcile()` 자동 해제(`expectedFix` seed 4건), **Invocation Contract L1~L5**: 226 assertions L1+L4 CI gate + L2 hook attribution 13 TC + L3 MCP stdio runtime 42 TC (실 spawn + tools/list) + L5 E2E shell smoke 5 scenarios, **Docs=Code CI** (`scripts/docs-code-sync.js`, 0 drift, 8 수치 + **BKIT_VERSION 5-location invariant**), **audit-logger 682GB recursion 근본 수정** (`createDualSink` 회피 + Integration Runtime TC 영구 방어), **Critical Bug C1/C2 fix**, **BKIT_VERSION 중앙화 완결** (ENH-167, `bkit.config.json` 단일 SoT), **ENH-202 `context: fork` 1→9 skills**, **legacy 3 modules removed** (421 LOC), **Domain purity CI** (`check-domain-purity.js`, 11 files / 0 forbidden imports), **PreCompact counter** (ENH-247/257 2-week 가동), **hook attribution** 3 sites (Stop/SessionEnd/SubagentStop). 6 Validator CLIs (`check-guards`, `docs-code-sync`, `check-deadcode`, `check-domain-purity`, `l3-mcp-runtime`, `test/e2e/run-all.sh`). Architecture: **39 Skills, 36 Agents, 21 Hook Events (24 blocks), 16 MCP Tools, 2 MCP Servers, 122 Lib Modules (~26,200 LOC), 47 Scripts, 112 test files, 3,741 TC**. CC recommended v2.1.117+ (**75 consecutive compatible releases**). Invocation Contract 100% 보존 + Starter/Dynamic/Enterprise zero-action update. MEMORY 302→79 lines 분해 (detail: cc_version_history_v21xx / enh_backlog / github_issues_monitor).
